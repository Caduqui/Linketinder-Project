...
class CandidatoSpec extends Specification{
    ...
    def "exibirDados mostra ID, nome, CPF e competências"() {
        when:
        String saida = SaidaConsole.capturar {
            criarCandidato().exibirDados()
        }
    ...
    def "exibirDadosAnonimos não mostra nome, e-mail nem CPF"() {
        when:
        String saida = SaidaConsole.capturar { criarCandidato().exibirDadosAnonimos() }
