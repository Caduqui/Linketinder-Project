...
class CadastroEmpresaSpec extends Specification{
    ...
    EmpresaDAO empresaDAO = Mock(EmpresaDAO)

    CadastroEmpresa cadastroCom(Map<String, String> respostas) {
        return new CadastroEmpresa(EntradaFalsa.comRespostas(respostas), empresaDAO)
    }
    ...
    def "cadastrar lê os dados digitados e insere a empresa"() {
        ...
        then:
        1 * empresaDAO.inserir({ Empresa empresa -> empresa.nome == "CloudX" && empresa.cnpj == "99.999.999/0001-99" })
        cadastrou
    }
    ...
    def "excluir não remove empresa que possui vagas"() {
        ...
        empresaDAO.buscarPorId(1) >> empresaExistente(1)
        empresaDAO.possuiVagas(1) >> true

        when:
        boolean excluiu = cadastro.excluir()

        then:
        0 * empresaDAO.deletar(_)
        !excluiu
    }
    ...
    def "excluir remove empresa sem vagas"() {
        ...
        CadastroEmpresa cadastro = cadastroCom(["ID da empresa: ": "1"])
        empresaDAO.buscarPorId(1) >> empresaExistente(1)
        empresaDAO.possuiVagas(1) >> false

        when:
        boolean excluiu = cadastro.excluir()

        then:
        1 * empresaDAO.deletar(1)
        excluiu
    }
    ...
    def "selecionarEmpresa retorna null quando a empresa não existe"() {
        ...
        empresaDAO.buscarPorId(42) >> null
